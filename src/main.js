...
function createPalm(x, z, scale = 1) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    leaf.position.set(Math.cos((i / 5) * Math.PI * 2) * 0.5, 6 * scale, Math.sin((i / 5) * Math.PI * 2) * 0.5);
...
function createUmbrella(x, z, hue = 0xff6fa1) {
  const group = new THREE.Group();
  const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 3.2, 10), new THREE.MeshStandardMaterial({ color: 0xf4f6f7 }));
  pole.position.y = 1.6;
  pole.castShadow = true;
  group.add(pole);
  const cap = new THREE.Mesh(new THREE.ConeGeometry(2.2, 1.2, 18), new THREE.MeshStandardMaterial({ color: hue, roughness: 0.8 }));
  cap.position.y = 3.6;
  group.add(cap);
  const towel = new THREE.Mesh(new THREE.BoxGeometry(2.8, 0.08, 1.6), new THREE.MeshStandardMaterial({ color: 0xf5ece0 }));
...
function createFirePit(x, z) {
  ...
  const flame = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xff9640, emissive: 0xff7128, emissiveIntensity: 1.8, transparent: true, opacity: 0.95 })
  );
...
function createComingSoonMarker(x, z, label = "COMING SOON") {
  ...
  const ring = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x54c2ff, emissive: 0x1560c5, emissiveIntensity: 0.45 })
  );
  ring.rotation.x = Math.PI / 2;
  group.add(ring);
  ...
  const text = createTextBillboard(label, { fill: "#07131d", accent: "#59b9ff", width: 9.5, height: 2.8 });
...
function createZoneLabel(text, x, y, z) {
  const label = createTextBillboard(text, { fill: "#0c1622", accent: "#7ce0ff", width: 8.2, height: 2.5 });
...
function createBeachBase() {
  ...
  const socialDeck = new THREE.Mesh(new THREE.BoxGeometry(28, 0.24, 16), new THREE.MeshStandardMaterial({ color: 0xf0cb8b, roughness: 0.95 }));
  socialDeck.position.set(-30, 0.12, 10);
  socialDeck.receiveShadow = true;
  world.add(socialDeck);

  const npcDeck = new THREE.Mesh(new THREE.BoxGeometry(18, 0.24, 18), new THREE.MeshStandardMaterial({ color: 0xf0cb8b, roughness: 0.95 }));
  ...
  const boundaryMat = new THREE.MeshStandardMaterial({ color: 0xd9eef7, transparent: true, opacity: 0.25 });
...
function createProps() {
  ...
  [[-36, 24, 1], [-26, 24, 1], [-18, 24, 1]].forEach(([x,z]) => createUmbrella(x, z, 0xff8ab1));
  [[-40, 0, 0], [-34, 0, Math.PI/2], [-24, 2, Math.PI]].forEach(([x,z,r]) => createBench(x, z, r));
  createFirePit(-30, 8);

  [[12, -18, 1.2], [18, -24, 1.0], [6, -26, 0.9], [62, 8, 1.2], [-58, -18, 1.05], [48, 40, 1.1]].forEach(([x,z,s]) => createRock(x,z,s));
  ...
  const portal = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x7ae3ff, emissive: 0x1b87d0, emissiveIntensity: 0.8 })
  );
...
function createNPC(name, x, z, color = 0xff7ab5) {
  const group = new THREE.Group();
  const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.9, 1.7, 5, 12), new THREE.MeshStandardMaterial({ color, roughness: 0.65 }));
  body.position.y = 2;
  body.castShadow = true;
  group.add(body);

  const head = new THREE.Mesh(new THREE.SphereGeometry(0.75, 18, 16), new THREE.MeshStandardMaterial({ color: 0xfff3ea }));
  head.position.y = 3.7;
  head.castShadow = true;
  group.add(head);
  ...
  group.position.set(x, 0, z);
  group.userData = { home: new THREE.Vector3(x, 0, z), timeOffset: Math.random() * Math.PI * 2 };
  addShadow(x, z, 0.5);
  world.add(group);
...
window.addEventListener("pointerup", () => { dragLookActive = false; });
...
function setPlayerName(name) {
  const clean = (name || "Guest").replace(/[^a-z0-9_\\- ]/gi, "").trim().slice(0, 16) || "Guest";
...
function animatePlayer(delta, time) {
  ...
  if (input.lengthSq() > 0) {
    ...
    const move = camForward.multiplyScalar(-input.z).add(camRight.multiplyScalar(input.x)).normalize().multiplyScalar(speed * delta);
...
function animateWater(time) {
  ...
  for (let i = 0; i < arr.length; i += 3) {
    ...
    const by = base[i + 1];
    const bz = base[i + 2];
    arr[i + 1] = by + Math.sin((bx + time * 16) * 0.035) * 0.52 + Math.cos((bz - time * 18) * 0.028) * 0.45;
  }
