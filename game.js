...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(this.scale.width / 2, 180, `Best Score: ${this.state.bestScore}`, {
      fontFamily: "Arial",
      fontSize: "22px",
      color: "#ffe08a"
    }).setOrigin(0.5);
    ...
    this.makeButton(this.scale.width / 2, 320, "Spiel starten", () => {
      this.scene.start("RunnerScene");
    });

    this.makeButton(this.scale.width / 2, 395, "Skin wechseln", () => {
      this.cycleSkin();
    });
  ...
  getSkinLabel() {
    ...
    return `Aktiver Skin: ${current.name}\nFreigeschaltet: ${this.state.unlockedSkins.length}/${SKINS.length}`;
  }
}
...
class RunnerScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(this.scale.width - 18, 18, "ESC = Menü", {
      fontFamily: "Arial",
      fontSize: "18px",
      color: "#cccccc"
    }).setOrigin(1, 0);
  ...
  spawnObstacle() {
    ...
    const obstacle = this.obstacles.create(this.scale.width + 40, this.scale.height - 142, "obstacle");
  ...
  spawnCoin() {
    ...
    const coin = this.coinGroup.create(this.scale.width + 40, Phaser.Utils.Array.GetRandom(yOptions), "coin");
  ...
  hitObstacle() {
    ...
    if (this.score > previousBest) message += `\n\nNeuer Highscore!`;
    if (unlockedAnything) message += `\nNeuer Skin freigeschaltet!`;
  ...
  update() {
    ...
    this.obstacles.children.each((child) => {
      if (child && child.x < -60) child.destroy();
    });
